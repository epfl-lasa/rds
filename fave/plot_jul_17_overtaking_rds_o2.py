...
folder_path = "./jul_17_row_overtaking_rds_o2/"
picture_export_prefix = "./pictures/jul_17_row_overtaking_rds_o2_"
...
subrange = np.arange(0, xy_p_ref_mat['data'].shape[0], 1) # np.arange(0, 450, 1)
...
plot_lrf = False
lrf_sampler = np.arange(0, i_d_min_2+1, 20)
...
if plot_lrf:
	...
	for k in range(lrf_points_sampled.shape[0]):
		plt.scatter(lrf_points_sampled[k,even].flatten(), lrf_points_sampled[k, odd].flatten(), color='k')
	#plt.scatter(lrf[0, even].flatten(), lrf[0, odd].flatten())
	#plt.scatter(lrf[i_d_min_2, even].flatten(), lrf[i_d_min_2, odd].flatten())
#np.savetxt("debug_tracks_matrix", tracks[:,0:20])
...
plt.scatter(xy_p_ref[0:i_d_min_2, 0], xy_p_ref[0:i_d_min_2, 1], c=t[0:i_d_min_2]/(t[i_d_min_2]-t[0]), cmap=cm.hot_r, marker='o')#, edgecolors='k')
for j in range(25):
	plt.scatter(tracks[0:i_d_min_2,j*4], tracks[0:i_d_min_2,j*4+1], c=t[0:i_d_min_2]/(t[i_d_min_2]-t[0]), cmap=cm.hot_r, marker="^", edgecolors='k')
	plt.scatter(tracks[0:i_d_min_2,j*4], tracks[0:i_d_min_2,j*4+1], c=t[0:i_d_min_2]/(t[i_d_min_2]-t[0]), cmap=cm.hot_r, marker="^")
...
ax = plt.gca()
capsule.plot_at_pose(x_of_t(t_d_min_1), y_of_t(t_d_min_1), phi_of_t(t_d_min_1), ax)
capsule.plot_at_pose(x_of_t(t_d_min_2), y_of_t(t_d_min_2), phi_of_t(t_d_min_2), ax)
...
if False:
	...
	for j in range(1):
		for i in range(t.shape[0]):
			...
			rob_v_x = -np.sin(phi_of_t(t_d_min_1))*v_corrected[i]#v_cartesian_corrected_p_ref[i, 0]
			rob_v_y = np.cos(phi_of_t(t_d_min_1))*v_corrected[i]#v_cartesian_corrected_p_ref[i, 1]
			diff_x = ped_x - rob_x
			diff_y = ped_y - rob_y
