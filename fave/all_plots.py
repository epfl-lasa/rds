...
def plot_and_export(name, t_snapshot_1, t_snapshot_2, show_big_circle, x_lim, y_lim, remove_ground):
	...
	if plot_lrf:
		...
		if not remove_ground:
			...
			plt.scatter(lrf[i_d_min_2, even].flatten(), lrf[i_d_min_2, odd].flatten(), color="g")
		else:
			r_cutoff_squared = 9.0
			for i_now in [i_d_min_1, i_d_min_2]:
				...
				plt.scatter(X_lrf_now[i_cutoff], Y_lrf_now[i_cutoff], color="g")
