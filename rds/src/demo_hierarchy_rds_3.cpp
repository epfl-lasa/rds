...
/*
	for (auto& cap : capsule_agents)
	{
		...
		sim->addAgent(RVO::Vector2(p_ref_global.x, p_ref_global.y), 15.0f, 10, 5.0f, 5.0f,
			cap.rds_3_configuration.robot_shape.radius, cap.rds_3_configuration.v_max);
	}
	for (auto& cir : circle_agents)
	{
		Vec2 v_result;
		cir.transformVectorLocalToGlobal(cir.rds_3_configuration.robot_shape.center, &v_result);
		Vec2 center_global = cir.position + v_result;
		sim->addAgent(RVO::Vector2(center_global.x, center_global.y), 15.0f, 10, 5.0f, 5.0f,
			cir.rds_3_configuration.robot_shape.radius, cir.rds_3_configuration.v_max);
	}
*/
...
void simulate(const char* title, float screen_size, int simu_index, float corridor_width = 0)
{
	...
	{
		sim.addAgent(RVO::Vector2(0.f, 0.f), 15.0f, 10, 5.0f, 5.0f,
			cir.rds_3_configuration.robot_shape.radius + cir.rds_3_configuration.delta,
			cir.rds_3_configuration.v_max);
	}
...
int main(int argc, char** argv)
{
	...
	switch (simu_index)
	{
		case 0:
		{
			capsule_agents.push_back(RDS3CapsuleAgent(Vec2(0.2f, -5.f), 0.f, RDS3CapsuleConfiguration(1.f,0.05f,1.5f,
				Capsule(0.5f, Vec2(0.f, 0.2f), Vec2(0.f, -0.3f)), Vec2(0.f, 0.2f))));

			Vec2 p_ref(0.f, 1.f);
			/*circle_agents.push_back(RDS3CircleAgent(Vec2(0.f, 0.f), 0.f, RDS3CircleConfiguration(1.f,0.05f,1.5f,
				Circle(p_ref, 0.5f), p_ref)));
			circle_agents.push_back(RDS3CircleAgent(Vec2(2.f, 2.f), 0.f, RDS3CircleConfiguration(1.f,0.05f,1.5f,
				Circle(p_ref, 0.5f), p_ref)));
			circle_agents.push_back(RDS3CircleAgent(Vec2(0.2f, 2.f), 0.f, RDS3CircleConfiguration(1.f,0.05f,1.5f,
				Circle(p_ref, 0.5f), p_ref)));
			circle_agents.push_back(RDS3CircleAgent(Vec2(0.5f, 1.f), 0.f, RDS3CircleConfiguration(1.f,0.05f,1.5f,
				Circle(p_ref, 0.5f), p_ref)));
			circle_agents.push_back(RDS3CircleAgent(Vec2(0.3f, 1.5f), 0.f, RDS3CircleConfiguration(1.f,0.05f,1.5f,
				Circle(p_ref, 0.5f), p_ref)));*/

			//circle_agents.push_back(RDS3CircleAgent(Vec2(-5.5f, 0.f), 0.f, RDS3CircleConfiguration(1.f,0.05f,1.5f,
			//	Circle(p_ref, 0.5f), p_ref)));
			...
			for (int i = 0; i < 7; i++)
			{
				for (int j = 0; j < 7; j++)
				{
					circle_agents.push_back(RDS3CircleAgent(Vec2(-20.f + i*2.7f + (j%2)*1.5f, 0.f + j*2.7f),//(-4.f + i*2.7f + (j%2)*1.5f, 8.f + j*2.7f),
						0.f, RDS3CircleConfiguration(1.f,0.05f,1.5f, Circle(p_ref, 0.5f), p_ref)));
				}
			}
		...
		case 1: //unfinished
		{
			capsule_agents.push_back(RDS3CapsuleAgent(Vec2(40.f, 40.f), 0.f, RDS3CapsuleConfiguration(1.f,0.05f,1.5f,
				Capsule(0.5f, Vec2(0.f, 0.2f), Vec2(0.f, -0.3f)), Vec2(0.f, 0.2f))));

			Vec2 p_ref(0.f, 1.f);
			// static agent (circular wall)
			float inner_radius = 50.f;
			circle_agents.push_back(RDS3CircleAgent(Vec2(0.f, 0.f) - p_ref, 0.f, RDS3CircleConfiguration(1.f,0.05f,0.f,
				...
				while (true)
				{
					...
					for (auto& a : circle_agents)
					{
						if (!valid)
							break;
						Vec2 v_result;
						a.transformVectorLocalToGlobal(a.rds_3_configuration.p_ref, &v_result);
						if ((a.position + v_result - position_candidate - Vec2(0.f,1.f)).norm() <
							a.rds_3_configuration.robot_shape.radius + pedestrian_radius +
							a.rds_3_configuration.delta)
						{
							valid = false;
							break;
						}
					...
					if (valid)
					{
						circle_agents.push_back(RDS3CircleAgent(position_candidate - p_ref, 0.f, RDS3CircleConfiguration(1.f,0.05f,0.f,
		...
		case 2:
		{
			...
			capsule_agents.push_back(RDS3CapsuleAgent(Vec2(0.f, 0.f), -M_PI/2.f, RDS3CapsuleConfiguration(1.f,0.05f,1.5f,
			...
			for (int i = 0; i < (window_size - 2.f*pedestrian_radius)/bin_width; i++)
			{
				for (int j = 0; j < (corridor_width - 2.f*(pillar_radius + pedestrian_radius))/bin_width; j++)
				{
					...
					circle_agents.push_back(RDS3CircleAgent(position - p_ref,
						0.f, RDS3CircleConfiguration(1.f,0.05f,1.6f, Circle(p_ref, pedestrian_radius), p_ref)));
				}
			}
			// static agents (walls)
