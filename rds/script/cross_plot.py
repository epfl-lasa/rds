...
for m in [0, 1]:
	...
	ax.scatter(traces_ordered[:,0], traces_ordered[:,1], c=traces_ordered[:,2]/float(traces_max_index), cmap=the_cmap, marker='o', lw=0.1)#, s=10)
	ax.set_aspect("equal")
	ax.set_xlim([-2.7, 4.0])
	ax.set_ylim([-2.7, 4.0])
	ax.plot([-2.7, 4.0], [0.0, 0.0], 'k')
	ax.plot([0.0, 0.0], [-2.7, 4.0], 'k')
	...
	y_colorscale = np.linspace(1.25, 2.5, 20)
	x_colorscale = np.linspace(2, 2, 20)
...
plt.savefig('dynamic_systematic_plot.png', bbox_inches='tight', dpi=199)
plt.show()